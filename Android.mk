...
ifeq ($(PRODUCT_USE_DYNAMIC_PARTITIONS),true)
    ...
    ifeq ($(TW_ALLOW_REWRITE_SUPER_METADATA),true)
        TWRP_REQUIRED_MODULES += lpmake lpflash
        LOCAL_CFLAGS += -DTW_ALLOW_REWRITE_SUPER_METADATA=true
        ifneq ($(BOARD_SUPER_PARTITION_SIZE),)
                LOCAL_CFLAGS += -DBOARD_SUPER_PARTITION_SIZE=$(BOARD_SUPER_PARTITION_SIZE)
        endif
        ifneq ($(BOARD_SUPER_PARTITION_GROUPS),)
                LOCAL_CFLAGS += -DBOARD_SUPER_PARTITION_GROUPS="$(BOARD_SUPER_PARTITION_GROUPS)"
                LOCAL_CFLAGS += -DSUPER_GROUP_SIZE=$(BOARD_$(call to-upper,$(BOARD_SUPER_PARTITION_GROUPS))_SIZE)
                LOCAL_CFLAGS += -DSUPER_GROUP_PARTITION_LIST="$(BOARD_$(call to-upper,$(BOARD_SUPER_PARTITION_GROUPS))_PARTITION_LIST)"
        endif
    endif
...
ifneq ($(TW_LOAD_VENDOR_MODULES),)
    ...
    ifeq ($(TW_LOAD_PREBUILT_MODULES),true)
        LOCAL_CFLAGS += -DTW_LOAD_PREBUILT_MODULES
    endif
endif
...
ifneq ($(PRODUCT_PLATFORM),)
    LOCAL_CFLAGS += -DTARGET_BOARD_PLATFORM=$(PRODUCT_PLATFORM)
endif
