...
int TWFunc::Check_MIUI_Treble(void)
{
  ...
  if (StorageIsEncrypted())
      ...
      gui_msg(Msg(msg::kWarning, "fox_not_encrypted=* Storage is not encrypted"));
...
void TWFunc::Welcome_Message(void)
{
   if (Fox_Has_Welcomed > 0)
    ...
    gui_msg(Msg(msg::kGreen, "fox_welcome=Welcome to OrangeFox Recovery!"));
        ...
        gui_msg(Msg("fox_platform=[Platform]  : {1}")(EXPAND(TARGET_BOARD_PLATFORM)));
    #else
    ...
    gui_msg(Msg("fox_codebase=[Codebase]  : {1}, {2}")(Fox_Property_Get("ro.build.version.sdk").c_str())(FOX_CURRENT_DEV_STR));
        ...
        gui_msg(Msg("fox_settings=[Settings]  : {1}")(Fox_Home.c_str()));
    #endif
...
void TWFunc::FoxThemeCheck()
{
	...
	if (theme_ver == build_theme_ver) {
		...
		if (has_themes_dir)
			gui_msg(Msg(msg::kWarning, "theme_mismatch=* Themes version mismatch (old={1}; new={2})")(theme_ver)(build_theme_ver));
		else
		...
		DataManager::Flush();
		if (has_themes_dir) {
			gui_msg(Msg(msg::kWarning, "theme_reset_themes=* Resetting the themes..."));
			TWFunc::removeDir(FOX_THEME_PATH, false);
		}

		if (TWFunc::Path_Exists(FOX_NAVBAR_PATH)) {
			gui_msg(Msg(msg::kWarning, "theme_reset_navbar=* Resetting the navbar..."));
